// Define a mapping of NPC textures to dialogues
export const npcDialogues = {
  guard: [
    ["Bienvenue à Los Alamos, étranger."],
    ["Vous avez des affaires ici ?"],
  ],
  colonel: [
    [
      "Bienvenue à Los Alamos, jeune recrue. Nous sommes en train de réaliser quelque chose d'incroyable ici.",
    ],
    [
      "Le projet Manhattan vise à développer la première bombe atomique. La puissance de l'atome entre nos mains.",
    ],
    [
      "Nous avons rassemblé certaines des plus grandes intelligences scientifiques du monde pour ce projet.",
    ],
    [
      "Mais n'oublions pas les implications éthiques et morales de ce que nous créons ici.",
    ],
    [
      "Le célèbre physicien Robert Oppenheimer dirige cette opération. Il a une vision radicale pour l'avenir.",
    ],
    [
      "Soyez prudent lorsque vous parlez de nos travaux ici. La sécurité et le secret sont primordiaux.",
    ],
    [
      "N'ayez pas peur de poser des questions et d'apprendre. Vous faites maintenant partie de l'histoire.",
    ],
  ],
  femme1: [
    ["Bienvenue dans notre laboratoire de pointe !"],
    ["Nous travaillons sur des expériences passionnantes ici."],
  ],
  scientifique1: [
    ["Oh bonjour, comment ça va ?"],
    ["J'espère que vous profitez de votre séjour à Los Alamos."],
  ],
  scientifique2: [
    ["Ah, vous avez entendu parler du projet Manhattan ?"],
    ["C'est un projet secret pour développer la première bombe atomique."],
    [
      "Certains pensent que c'est dangereux, d'autres disent que c'est nécessaire.",
    ],
  ],
  indian: [
    [
      "Dans cette région désertique où se trouve Los Alamos, les Amérindiens ont une histoire riche et ancienne.",
    ],
    [
      "Les Pueblos, notamment les Pueblos Tewa, ont habité cette région depuis des siècles.",
    ],
    [
      "Ils ont développé des communautés prospères avec une culture et des traditions uniques.",
    ],
    [
      "Lorsque le projet Manhattan a débuté, de nombreuses terres amérindiennes ont été réquisitionnées, entraînant des perturbations importantes pour ces communautés.",
    ],
    [
      "Il est essentiel de se souvenir de leur histoire et de leur contribution à cette région.",
    ],
    [
      "Les Amérindiens de la région ont une profonde connexion avec la terre et la nature.",
    ],
    [
      "Ils ont transmis des connaissances ancestrales sur la faune, la flore et la survie dans ce désert aride.",
    ],
    [
      "Malheureusement, leur mode de vie a été grandement perturbé par l'arrivée des colons européens et les changements sociaux qui ont suivi.",
    ],
    [
      "Malgré cela, de nombreuses traditions et croyances autochtones continuent d'influencer la culture de cette région.",
    ],
    [
      "Prenez le temps d'en apprendre davantage sur leur histoire et leur héritage si vous en avez l'occasion.",
    ],
  ],
};
